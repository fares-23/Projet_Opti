...
def MonteCarlo():
    capacites = np.linspace(0, 14000, 1000)  # Capacité batterie en Wh
...
axs[0].scatter(best_capacite, best_chute, color="green", label="Meilleure solution")
...
note = f"Meilleure solution: Capacité = {best_capacite:.2f} Wh, Chute de tension = {best_chute:.2f} V, Seuil = {(best_seuil/1000000):.2f} MW"
fig.text(0.5, 0.01, note, ha='center', va='center', fontsize=10)
...
#Meilleure solution: Capacité = {best_capacite:.2f} Chute de tension = {best_chute:.2f}, Seuil = {best_seuil:.2f}")
